  ...
  return (
    <ul>
      {values.map((value, idx) => (
        <li key={idx}>
          {value ? <IRIField value={value} settings={settings} /> : ""}
        </li>
      ))}
    </ul>
  );
  ...
  return (
    <>
      <tr>
        <td colSpan="6">
          <strong>
            Properties from <IRIField value={resourceUri} settings={settings} />
          </strong>
        </td>
      </tr>
        ...
        return (
          <tr key={key}>
            <td>
              <strong>
                <IRIField
                  value={results[key]["http://www.w3.org/ns/shacl#path"][0]}
                  settings={settings}
                />
              </strong>
            </td>
            <td>
              <Description
                values={
                  results[key]?.["http://www.w3.org/ns/shacl#description"]
                }
                settings={settings}
              />
            </td>
            <td>
              <Cardinality
                max={results[key]?.["http://www.w3.org/ns/shacl#maxCount"]?.[0]}
                min={results[key]?.["http://www.w3.org/ns/shacl#minCount"]?.[0]}
              />
            </td>
            <td>
              <ExpectedValueTypes
                values={results[key]?.["http://www.w3.org/ns/shacl#nodeKind"]}
                settings={settings}
              />
            </td>
            <td>
              <ExpectedValueClassTypes
                classes={results[key]?.["http://www.w3.org/ns/shacl#class"]}
                datatypes={
                  results[key]?.["http://www.w3.org/ns/shacl#datatype"]
                }
                settings={settings}
              />
            </td>
            <td>
              <ExpectedValues
                values={results[key]?.["http://www.w3.org/ns/shacl#hasValue"]}
                settings={settings}
              />
            </td>
          </tr>
