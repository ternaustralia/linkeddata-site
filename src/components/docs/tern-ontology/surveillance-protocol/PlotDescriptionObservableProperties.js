...
select ?concept ?featureType (sample(?_label) as ?label) (sample(?_featureTypeLabel) as ?featureTypeLabel) ?valueType (sample(?_valueTypeLabel) as ?valueTypeLabel) ?categoricalCollection (sample(?_categoricalCollectionLabel) as ?categoricalCollectionLabel)
...
export default function PlotDescriptionObservableProperties() {
  ...
  const concepts = data.results.bindings.map(value => (
    <tr key={value.concept.value}>
      <td><ExternalLink href={value.concept.value}>{value.label.value}</ExternalLink></td>
      <td><ExternalLink href={value?.featureType?.value}>{value?.featureTypeLabel?.value}</ExternalLink></td>
      <td><ExternalLink href={value?.valueType?.value}>{value?.valueTypeLabel?.value}</ExternalLink></td>
      <td><ExternalLink href={value?.categoricalCollection?.value}>{value?.categoricalCollectionLabel?.value}</ExternalLink></td>
    </tr>
  ))
