...
- A `tern:RDFDataset` must exist and referred to via `void:inDataset` for certain objects (JSON object). See [Dataset Metadata](/tern-ontology/dev-guide/rlp-protocol/metadata) for more information on `tern:RDFDataset`.
...
<CodeBlock className="language-json">
  {JSON.stringify(jsonLdContext, null, 2)}
</CodeBlock>
